...
import{collection,deleteDoc,doc,getDocs,getFirestore,query,serverTimestamp,setDoc,where}from"https://www.gstatic.com/firebasejs/12.17.1/firebase-firestore.js";
...
onAuthStateChanged(auth,async user=>{if(!user)return;for(const [type,elementId,label]of[["IC","icReference","Select an IC"],["PSU","psuReference","Select a PSU"]]){try{const select=document.getElementById(elementId),snapshot=await getDocs(query(collection(db,"parts"),where("type","==",type))),records=snapshot.docs.map(item=>({id:item.id,...item.data()})).sort((a,b)=>String(a.recordNo||"").localeCompare(String(b.recordNo||"")));select.replaceChildren(new Option(label,""),...records.map(record=>new Option(`${record.recordNo||record.id} — ${record.brand||""} ${record.partNo||record.modelNo||""}`.trim(),record.id)))}catch(error){const select=document.getElementById(elementId);if(select){select.replaceChildren(new Option("Unavailable",""));select.disabled=true}}}});
