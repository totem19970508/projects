...
onAuthStateChanged(auth,async user=>{if(!user)return;for(const [collectionName,elementId,label]of[["ics","icReference","Select an IC"],["psus","psuReference","Select a PSU"]]){try{const select=document.getElementById(elementId),snapshot=await getDocs(collection(db,collectionName)),records=snapshot.docs.map(item=>({id:item.id,...item.data()})).sort((a,b)=>a.id.localeCompare(b.id));select.replaceChildren(new Option(label,""),...records.map(record=>new Option(`${record.id} — ${record.brand||""} ${record.partNo||record.modelNo||""}`.trim(),record.id)))}catch(error){const select=document.getElementById(elementId);if(select){select.replaceChildren(new Option("Unavailable",""));select.disabled=true}}}});
